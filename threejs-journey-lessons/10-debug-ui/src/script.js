...
// Object for sending some debug values to the library which need to be
// wrapped inside an object
const debugParamHelper = {
    color: 0xff0000, // no longer required by lil-gui. It is required as a 
    spin: () =>
    {
...
folder.add(mesh.position, 'y').min(- 3).max(3).step(0.01).name('Cube Y');
folder.add(mesh.position, 'z').min(- 3).max(3).step(0.01).name('Cube Z');
